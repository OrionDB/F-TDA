...
class DbAccess {
    ...
    public function getSubjectsByForumName($forName){
        ...
        // Define the Sql Request
        $req = "Select idSubject, subTitle, idMember, memPseudo, graColor from t_subject natural join t_forum natural join t_member natural join t_grade where forName = '$forName'";
    ...
    public function getLastPostInSubject($subName){
        ...
        $req = "SELECT posDate,  memPseudo, subTitle, graColor FROM t_subject left outer join t_post on t_post.idSubject = t_subject.idSubject inner join t_member on T_post.idMember = t_member.idMember natural join t_forum natural join t_grade where posIsDeleted = 0 AND subTitle = '$subName' order by posDate desc Limit 1";
	...
	/**
	 * This function will return the name and the date of the last post contains in the forum
	 * @param $forName = This is the Forum name
	 * @return array
	 */
	public function getLastPostInForum($forName){

        $req = "SELECT(SELECT memPseudo FROM t_subject left outer join t_post on t_post.idSubject = t_subject.idSubject inner join t_member on T_post.idMember = t_member.idMember natural join t_forum natural join t_grade where posIsDeleted = 0 AND forName = 'Informations' order by idpost desc LIMIT 1) as subTitle, (SELECT posDate FROM t_subject left outer join t_post on t_post.idSubject = t_subject.idSubject inner join t_member on T_post.idMember = t_member.idMember natural join t_forum natural join t_grade where posIsDeleted = 0 AND forName = '$forName' order by idpost desc LIMIT 1) as Test";
        $result = $this->executeSqlRequest($req);
        return $result;
    }// End getLastPostInForum
